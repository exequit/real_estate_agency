...
class Flat(models.Model):
    ...
    living_area = models.IntegerField(
        "количество жилых кв.метров", null=True, db_index=True)

    has_balcony = models.NullBooleanField(
        "Наличие балкона", null=True, db_index=True)
    active = models.BooleanField("Активно-ли объявление", db_index=True)
    construction_year = models.IntegerField(
        "Год постройки здания", null=True, db_index=True)
...
class Complaint(models.Model):
    ...
    user = models.ForeignKey(
        User, verbose_name='Кто жаловался', on_delete=models.SET_NULL, blank=True, null=True)
    flat = models.ForeignKey(
        Flat, verbose_name='Квартира, на которую жаловались', on_delete=models.CASCADE)
    description = models.TextField("Текст жалобы", default="")

    def __str__(self):
        return "".format(self.user.name, self.flat.town, self.flat.address, description[:100])
